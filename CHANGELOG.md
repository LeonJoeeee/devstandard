...
- **This plugin retires its own translations; the mirror rule stands for target projects** (PR #95, issue #94, ADR 0028 amending 0023, 0024 and 0027). `core.zh-CN.md` and `README.zh-CN.md` are deleted, along with the CI gate that enforced the `core.md` pairing and the `fetch-depth: 0` checkout only that gate needed. The repository is English throughout. **`core.md` is untouched** — 4,560 of 5,000 — but every future rule change now costs one edit instead of two, which is the real saving on a page with 440 tokens of headroom left. CI drops from five gates to four, all of which can now fail for the reason they claim.
- **Why, measured rather than assumed.** `hooks/session-start` names only `core.md`; a grep of the tree for anything that *opens* the translation returned two hits — a human's link in `README.zh-CN.md`, and the gate itself. The translation was in no read path the method prescribes. Agents did read it, but only as maintainers keeping it faithful and reviewers checking that they had: **cost, not audience.** And the gate could not check what the rule required — it asserted that `core.zh-CN.md` appears in any diff touching `core.md`, which is *co-modification, never agreement*. Prose has no mechanical equivalence test, so it was a presence check wearing a correctness check's name. A check-1 round on PR #85 found the Chinese *stronger* than the English it mirrored, passed green by the gate that existed to prevent exactly that.
- **ADR 0023's rule is scoped, not repealed.** A target project whose record is in another language still declares it in its repo-root `CLAUDE.md`, and a translation it keeps for humans is still a marked mirror naming its canonical file and riding the same diff. `core.md:71` carries that rule and is unchanged. The distinction, recorded so it is not re-derived: 0023's mirror serves a project whose *record* is another language, where the canonical is the translation's source and **both have readers** — here the canonical had every reader and the mirror had a courtesy copy, so the cost landed entirely on the side that got no benefit.
- **One rule came out of the sweep rather than the decision**, and it sharpens ADR 0027's historical-record carve-out: **an ADR body that says what was true when the decision was made is history and is never reconciled; one that says what a *future* action will cost is a live instruction and is.** The worked case is ADR 0024, whose "a rename or retirement … costs exactly four edits: core.md, core.zh-CN.md, …" would have sent a future session looking for a deleted file.
- Method note: five review rounds on a seven-file diff, four of them earned. Rounds 1–3 each found the previous round's fix leaving the same claim standing in a file the same commit was already editing — including two false claims heading for the permanent decision log. Three independent reviewers verified the gate removal from `main` rather than inheriting the earlier clearance, which is the part green CI can never vouch for. ADR 0027 shipped eight days earlier against exactly this failure shape, and its first four applications each demonstrated it again.

## 0.15.0

- **Rewording a rule is its own doc duty — and it takes two searches, not one** (PR #85, issue #79, ADR 0027 amending 0022). The doer's doc duty reaches documents a change made *false*. It does not reach a document that states the same rule *differently*, or that *cites* it — and issue #79 counts six instances of the resulting staleness and describes five, four of them inside a single PR, each found by a different party. Every one of those fixes was verified in place; "in place" is the defect, because it means the two files in hand. The sixth instance shows why one search cannot work: `core.md` was widened from "any rebase" to "any rebase **or amend**", leaving `howto/cicd.md` telling every seeded project to re-run check 1 "after any rebase" — and the implementer *and* the round-1 reviewer both swept for the word they had added, a search that finds every site already saying it and by construction cannot find the sites that should now say it. **Pass 1:** every other statement of the clause — the mirror, the aids, the howtos, the architecture doc, and the ADR that recorded it. **Pass 2:** every site that cites or paraphrases it, found by its *pointer* to the rule, never by the new words, because widening a rule silently narrows every summary of it. Each site is reconciled **or explicitly cleared with a stated ruling in the PR description** — a site simply absent from the sweep is a silent omission, not a clearing.
- **Two kinds of site take a specific form, because "reconcile" otherwise reads as "edit".** An **ADR** is reconciled by appending a dated amendment block, never a rewritten body — the append-only record is the property the whole log exists to hold, and the rule as first written would have sent three different readers to edit one. A **historical record** (a CHANGELOG entry, a merged PR description) is not a site at all: it states what was true at that release, and "correcting" it falsifies the history rather than the rule.
- It lands in three places for three readers: `core.md` (+180 tokens, 4,560 of 5,000) because a main session on its own short branch reads no aid; `aids/worker-brief.md` because a worker fixing a check-1 finding is the commonest rewriter and never receives `core.md`; `aids/code-review-prompt.md` because the reviewer caught most of the six — the backstop stays a backstop, now told what to look for.
- Carried fixes, both demonstrations of the rule: `howto/cicd.md:45` becomes "after any rebase **or amend**" (the sixth instance itself), and `howto/adr.md` drops this repo's own issue numbers from a page that ships to seeded projects, where `#39` resolves to an unrelated local issue. Swept for the same defect in the same surfaces: two ADR-number citations in `howto/adr.md` and one in `aids/code-review-prompt.md`, all dropped, substance kept — every repo-internal identifier is now gone from the pages that ship into a target repo.
- Method note: three review rounds, and **rounds 2 and 3 each found staleness introduced by the previous round's fix** — the defect this change exists to close, demonstrated on itself several times over. Round 1's Critical (the rule told three readers to reconcile an ADR without the never-rewrite guard) and round 2's Important (the checklist silently dropped the architecture doc, the one document every session and every worker is told to read) are both design-level holes a pre-code challenge is meant to catch; this change disclosed that it skipped one, and ADR 0027's own Rejected (a) argues against paying in review rounds what a challenge costs once. Two follow-ups filed rather than folded in: #86 (the READMEs advertise `core.md` at "~3,000 tokens" against an actual 4,560) and #87 (the sweep checklist names this plugin's own directory layout to readers who have neither).
