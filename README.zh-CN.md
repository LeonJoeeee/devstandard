...
- **说一句"建个库",完整生命周期就位** —— PRD → 架构文档 + 决策日志 → 钉死接口的薄骨架 → CI + tag 触发的发布流水线 → 拆任务、扇出并行 session。所有项目一视同仁,不做大小猜测。
- **已有仓库里的改动就是一个任务** —— 不写文档、零仪式,但纪律照常生效。
- **每个任务都有纪律** —— 写代码前先有机器可判的完成标准;设计先被独立的干净上下文评审驳过;同一时刻只有一个写者(并行火力给验证);"做完了"必须带命令、退出码和输出。
- **并行不撞车** —— 一个 session = 一条分支 = 一个 worktree = 一个任务;session 交付分支/PR;**merge 归 `main`**,CI 绿灯把门;动架构还要 maintainer 批准 + 记一笔决策日志。
- **它很便宜** —— 每个 session 一页纸(约 700 token);模板和辅助工具只在真被读时才进上下文。没有后台进程、不依赖外部服务、不需要其他插件。
...
**大项目并行推进** —— 每个任务开一个 Claude Code session;各占一条分支和 worktree,开工前先读架构文档。session 推分支;CI 绿了**由你** merge。哪个任务要动架构本身,它会回到你面前:公开 merge、你批准、文档更新、决定留痕。其他人——带着他们自己的 agent——走完全相同的流程加入。
...
**会拖慢小改动吗?**
不会。完整生命周期只在你要求新建仓库时触发(明确信号,不做复杂度猜测,[ADR 0004](docs/adr/0004-trigger-repo-creation-uniform-lifecycle.md))。其余一切都是任务。

**到底有什么进了我的上下文?**
[`core.md`](core.md),每 session 一次,约 700 token。其余文件除非被 agent 明确读取,否则不进。
