...
2. **项目记忆** —— 每个项目一套 PRD、架构文档、决策日志、实质性改动的设计规格,和一份仓库 CLAUDE.md(命令与环境坑),让并行的 session(以及人类队友)在"做什么、怎么做、为什么"上始终对齐;
...
**开新项目** —— 说 *"给 X 建一个新仓库"*。agent 会把你访谈成一页 PRD(做什么/为什么/什么算做完——你批准),写下以后每个 session 都当共同地图用的架构文档,起好决策日志,搭出骨架,生成 CI + 发布流水线和一份仓库根的 CLAUDE.md(命令与环境坑,以后每个 session 自动读),然后把活拆成任务。
...
**能用在已有项目上吗?**
能。改动从第一天起就是任务;文档(`docs/PRD.md`、`docs/architecture.md`、`docs/adr/`、仓库根 `CLAUDE.md`)什么时候想补都可以——模板在 `howto/`。
