...
**If CI cannot run at all** — no push produces a run (minutes exhausted, provider outage) — check 2 degrades: the merging session, never the worker, runs every CI job on the merged result and posts the evidence, the merge SHA and proof of the outage on the PR, for check 1 to audit before merging. Not triggers: slow, queued, flaky or red CI, or a workflow this repo could fix; unsure means it can run. A never-reporting required check blocks the merge — the human unblocks it, never you. It ends at the first push that runs; main's next green CI re-verifies what merged under it (`howto/cicd.md`).
