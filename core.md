...
- Ask the human ONLY when the change touches top-level design, the action costs a lot (e.g. a workflow run or many parallel agents), or the action is destructive or hard to undo (deleting data, force-pushing a branch others depend on — main, a shared branch, one a review is in flight against — anything leaving the repo: publishing, sending). `--force-with-lease` on your own unmerged branch is ordinary work — amending after check 1 passed re-runs check 1. Otherwise act on your own. When unsure, treat it as big and ask.
