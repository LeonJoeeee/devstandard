...
3. The doer works on a branch (a dispatched worker also gets its own worktree): build → update the docs the change invalidates (they ride the same diff; a reworded rule sweeps twice — below) → rebase onto current main → run the done-check on the final state and capture evidence → push, open a PR (linked to the issue, when there is one), and drive it green.
...
**The doer's doc duty is universal:** whoever makes a change — any venue, any size, a main-session small fix included — updates the docs that change invalidates, in the same diff (architecture/PRD escalate first). The reviewer's Docs check is the backstop, not the first line. **Rewording a rule is its own doc duty:** the same clause lives in several places, so search twice — for every other statement of it (the mirror, the aids, the howtos, the architecture doc, the ADR that recorded it), and for every site that cites or paraphrases it, found by its pointer, never by the words you just added: the sites that need them are the ones that lack them. Reconcile each in the same diff or say in the PR description why it needs none — an ADR by appending a dated amendment block, never a rewritten body (`howto/adr.md`), and a historical record (a CHANGELOG entry, a merged PR description) not at all: it says what was true then.

**Opening a PR isn't done:** its opener owns it until every check reports green and every review-bot finding is fixed or answered on the PR. A bot finding is an opinion; a red check is a gate — fix the cause, never loosen the check. A doer returning before a check reports hands back the PR link and that check, never one it watched go red; the main session inherits at delivery and takes what can never go green to the human. (`howto/cicd.md`)
...
- DO: work only in your branch/worktree; build the design that survived the challenge; update any doc your change invalidates — docs ride the same diff, and a reworded rule sweeps twice (architecture/PRD escalate first); before delivering, `git fetch` and rebase onto current main, fixing your own conflicts; run the done-check and capture the evidence (commands, exit codes, output); push and open a PR linked to the issue, and drive it green.
...
- DONE for you: a PR open and linked to the issue, rebased clean on current main, evidence in the description, its checks reported green or handed back unreported, never red. Review and merge are the main session's job, not yours. Leave your worktree in place — the main session removes it when it merges.
