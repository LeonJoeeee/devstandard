...
|---|---|---|---|---|---|
| Worker | `devstandard:worker`'s body is `reference/worker.md` verbatim, so the harness carries the role as the subagent's system prompt with no read (ADR 0060); its frontmatter fixes the skill and hook bindings, and dispatch supplies the issue, lane and explicit purpose-routed model/effort. | Dispatch supplies the full shared role and task, worktree and explicit model/effort for an actual native spawn with fresh conversation. Developer instructions, cwd and permissions remain inherited. | Dispatch supplies the same role/task, explicit settings and a sandbox granting the worktree plus required linked-worktree git metadata. | Explicit worker process with the same role/task and assigned worktree, using host/tool permissions and noninteractive `acceptEdits`, without a sandbox bypass. | A green PR linked to the issue, rebased on current `main`, with final-state evidence. |
...
`agents/reviewer.md`. `agents/worker.md` is hand-authored frontmatter plus a body generated verbatim
from `reference/worker.md`, which stays the single hand-written source; `.github/check-agents.py`
asserts the two are byte-identical and regenerates the body under `--write`. A role's own subagents
...
| Amended for role delivery | 0007, 0015, 0016, 0019 → 0049; 0015, 0036, 0040 → 0047; 0024 dated block; 0024, 0040 → 0050 | Static context is now one delivered artifact per role, injected by default with the carrier chosen from a measured size and a CI gate that fails an over-cap artifact (0049). The dispatch-first rule and the retirement of the ladder's rung vocabulary ride 0047. ADR 0050 supersedes 0024's cap and 0040's restatement with the model/effort ladder by kind of work in `reference/orchestrator.md`'s Dispatching to an executor section, including nested helpers. ADR 0045 reconciled the Codex host removal earlier. |
