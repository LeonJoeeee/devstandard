...
| ⑤ merged result → ⑥ delegated release | **Hard:** the PreToolUse role hook refuses an orchestrator command whose text carries `tag` or `release` unless the repository's default-branch policy relays a standing delegation. **Structural:** the orchestrator set requires the one-line report after release. **Soft:** the human decides a new delegation or major-release sign-off. | **Unverified:** the blocking hook and its covered word list have not been exercised live. Since ADR 0051 there is no per-release record to look up. This is the mechanizable boundary around PRD §1.3. |
...
| An irreversible action is needed | **Hard:** workers and reviewers lack merge, release, and external destructive capabilities; the orchestrator's PreToolUse hook refuses `gh pr merge`, `git merge` and a push naming the default branch, routing merges through `guard merge`. **Soft:** the orchestrator identifies every irreversible action the hook's word list does not reach — which since ADR 0051 is deliberately most of them, teardown of a merged lane included. | **Unverified:** capability cuts and live hook enforcement. Addresses PRD §1.3. |
| Architecture-level change or major release is ready | **Hard:** `guard merge` requires the durable human sign-off record for an architecture-level merge — since ADR 0051 the only record left; a release rides the standing delegation in default-branch policy instead. **Soft:** the orchestrator classifies the change and the human decides. | **Unverified:** marker shape and guard integration. Addresses the irreversible-control concern in PRD §1.3. |
...
| Returned for a goal fix | **Structural:** a continuation brief carries the named goal gap into the same branch, worktree, and PR; the lane persists and the executor is disposable. **Hard:** per-PR round accounting enforces the 7-round cap. | **Verified — [issue #179's round ruling](https://github.com/LeonJoeeee/devstandard/issues/179#issuecomment-5525395030):** same-lane continuation and the cap are settled. **Unverified:** continuation dispatch and mechanical round accounting. |
...
branch protection and the OS sandbox. A worker's required access to shared git metadata still does
not by itself enforce the named-branch boundary.
...
| PreToolUse role hook | §1.3 | Refuses the ordinary spelling of the operations a role must never perform, cheaply enough to hold in one's head and without refusing ordinary work, except the founding push a repository whose default branch carries no policy file cannot otherwise make. |
...
shipped pages unable to become guarded at all. The admission is an orchestrator's push naming the
default branch, and only while that branch carries no policy file. **Its protection half was
dropped on 2026-09-10 (#323)** with the remote reads: proving a branch unprotected needed a GitHub
call inside a tool call, and GitHub's own protection rejects the push server-side anyway.
`reference/hard-edges.md` owns its wording and its limits; ADR 0046's 2026-09-07 and 2026-09-10
blocks record the amendment and this narrowing.
