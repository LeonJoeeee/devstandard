...
> Shared baseline for all parallel work. Read before any task. Changing anything here requires a
> public PR and an ADR; a direction change returns to the human.
...
superseding ADR 0045's removal. Both hosts use the same self-contained role sources. Claude Code retains
...
|---|---|---|---|---|---|
| Worker | `devstandard:worker` fixes static role and skill bindings; dispatch supplies the issue, lane and explicit purpose-routed model/effort. | Dispatch supplies the full shared role and task, worktree and explicit model/effort for an actual native spawn with fresh conversation. Developer instructions, cwd and permissions remain inherited. | Dispatch supplies the same role/task, explicit settings and a sandbox granting the worktree plus required linked-worktree git metadata. | Explicit worker process with the same role/task and assigned worktree, using host/tool permissions and noninteractive `acceptEdits`, without a sandbox bypass. | A green PR linked to the issue, rebased on current `main`, with final-state evidence. |
...
dispatch prompts are delivery carriers, not independently edited copies. During the two-step role
split, `core.md` temporarily holds worker-addressed triggers and remains a compatibility artifact
beside the orchestrator page; orchestrator authority nevertheless has one source. Step 2 removes
the remaining mis-addressed delivery (PRD §1.5, §1.6; ADR 0059).
...
The role-specific source split applies on both hosts. Codex adds only its bounded adapter artifact and an
explicit skill entry for hookless instruction recovery. The skill is not a self-triggered delivery
guarantee and cannot enable a guard whose hooks are untrusted. **Per source:** during step 1,
startup and clear still deliver the temporary core and self-contained orchestrator page; Codex
compaction does too. Claude Code compaction delivers `core.md` alone because a Claude Agent child's compaction fires the main-session SessionStart
...
[issue #375](https://github.com/LeonJoeeee/devstandard/issues/375)); core's neutral role-source
pointer lets a root orchestrator recover its page without adding orchestrator authority there. The adapter also fires
on resume and requires full reads of any role sources missing from an older session. Codex native workers receive
...
|---|---|---|
| Orchestrator static set | During step 1, Claude Code's SessionStart hook delivers the temporary core and self-contained orchestrator page as separate artifacts, inline by default with an instructed read selected only from each measured size. The same trigger repeats after a clear. On compaction the ambiguity-safe `core.md` carrier fires alone. | **Verified — repository source:** `hooks/hooks.json`, `hooks/session-start`, and the local CI hook gates show one output per delivered artifact—the complete artifact inlined when it fits the measured cap and the IN FULL read only when it does not—and the compact source delivering `core.md` rather than the orchestrator page (#258, #375; ADR 0049). Native startup is qualified below; persistent UI lifecycle behavior remains **Unverified**. |
| Codex orchestrator static set | Trusted plugin hooks deliver the temporary core, self-contained orchestrator page and `reference/harness-codex.md`. The adapter's resume trigger instructs reads of missing role sources. The explicit `devstandard` skill reads those sources when hooks are unavailable. | **Verified — native startup:** see **Codex host qualification (#342)** below. Persistent UI lifecycle behavior remains **Unverified**; manual invocation does not demonstrate automatic delivery. |
...
| A merge or major release is ready | **Structural:** the orchestrator set records how far the issue handover reaches and keeps release authorization separate; the review packet still routes architecture-level review one tier higher. **Soft:** the human owns a merge absent `delegated`, and every major release needs their explicit direction. | **Verified — repository source:** `reference/orchestrator.md` supplies both authority rules and `reference/external-agent.md` supplies review routing. Addresses the irreversible-control concern in PRD §1.3. |
...
CLI implementations use a fresh process. Before native continuation or cleanup,
...
4. Split the orchestrator and worker context into two role references, evolving the existing worker
   brief. ADR 0059 subsequently makes each page self-contained: step 1 moves orchestration out of
   `core.md`, and step 2 moves its temporary worker triggers into the worker page and deletes it.
   Bind superpowers once per role. Issue #200 removed Codex host delivery during this rebuild; ADR
   0056 restores it using the role sources, with workers still receiving their role through dispatch
   (PRD §1.5, §1.6, §2.3).
...
| Self-contained role sources | 0059 | Replaces the shared-core-plus-supplement shape in two self-consistent steps so each role receives only its own authority and workflow. |
