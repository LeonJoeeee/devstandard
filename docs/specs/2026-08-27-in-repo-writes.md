...
THE RECORDED OVERRIDE BELOW APPLIES TO EVERY ROW THAT NAMES A LOCATION: where the human decided,
before the write, where something goes, that decision is the destination — see the ladder. Rows 3, 7
and 8 say "stop and ask" precisely so that decision can be obtained; the ask must have a return path.
 ...
 6. Data, an artifact or a log you must keep — GENERATED by the task or ACQUIRED for it (a downloaded
    ...
    (row 7), not a service's persistent state (row 8). -> the authority ladder below. A
...
<!-- END PLACEMENT TABLE (65 payload lines) -->
```
