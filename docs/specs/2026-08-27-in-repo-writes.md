...
outranks how it was produced.
 ...
 1. Material the repository MAINTAINS — source; configuration the product ships and the repo's own
    ...
    output does not make it this line: a generated results.json is line 10 however you store it.
 ...
 5. One-time evidence you show someone — a screenshot, a coverage summary, a benchmark number.
    ...
    sensitive on a channel that should not carry it. Anything that must also be KEPT is line 10.
 6. Reusable tool-managed material outliving the task — model weights, a shared venv, a cloned tool,
    a ccache. -> the tool's own cache (`out-of-repo-writes.md` kind 1). A one-off fetch is line 9.
 7. State a service owns. -> `out-of-repo-writes.md` kind 2, at a root the repo's docs declare.
 8. Tool-managed working output inside the worktree — node_modules/, .venv/, .pytest_cache/, a build
    directory the tool requires. -> the ignored location that tool owns, never an agent-chosen one.
 9. Dies with the task — intermediates, a report read once, a dispatched process's output file.
    -> the scratch your session provides, or one `mktemp -d`.
10. Something that must be KEPT — data, an artifact, a log, generated by the task or acquired for it.
...
> path the repo or the human named, is fine and is not this; (ii) a destination whose only authority
> is something this same diff added; (iii) a kept file left in a worktree that the PR does not name;
