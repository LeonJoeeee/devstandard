...
would let edits made during the work count as pre-existing.
 ...
 3. Local configuration or secrets the task needs — .env.local, a key, seeded data the repo expects.
    ...
    tool documents (a declaration this diff adds licenses nothing); where neither names one, the recorded human override, else stop-and-tell (a worker) or ask (the main
    session). A
 ...
 4. A release deliverable — a wheel, installer, container image, release archive. -> the repo's release and publishing convention AS IT
    STOOD AT THE PINNED BASE, or the recorded human destination; a release workflow this diff adds is
    not a convention. Never committed as a by-product, never merely attached to an
...
THE RECORDED OVERRIDE APPLIES TO EVERY ROW THAT NAMES A LOCATION, AND CHANGES ONLY THE DESTINATION:
where the human decided, before the write, where something goes, that decision is where it goes. It is
applied AFTER the row is chosen and waives nothing else the row requires — not row 2's admission test,
not row 3's never-committed rule, not row 5's safe-channel and redaction rules, not row 6's
outlive-the-task requirement. Rows 3, 7
and 8 say "stop and ask" precisely so that decision can be obtained; the ask must have a return path.

 6. Data, an artifact or a log you must keep — GENERATED by the task or ACQUIRED for it (a downloaded
    corpus, a database snapshot, a model you manage by hand) — not a release (row 4), not reusable tooling material
    (row 7), not a service's persistent state (row 8). -> the authority ladder below. If it carries
    personal data, secrets, or licensed material it is NOT committed and NOT published by default:
    redact it, or use a protected destination authorised in advance for that content; where neither is
    possible, stop and ask. A
 ...
 7. Reusable, non-secret, tool-managed material that outlives the task — fetched or generated: model
    ...
    ladder's arms (a)-(c) or the recorded human override: a cache this task created is not a
    convention. A one-off fetch is row 10; a fetched credential is row 3.
 8. State a service owns, whether it persists or not. PERSISTENT state -> out-of-repo-writes.md
    kind 2, at a root declared in the repo's docs before this work, or the recorded human override; never a
    root this diff declares. EPHEMERAL runtime files a running service needs — a PID file, a socket, a request
    spool, a disposable cache -> the location the service's configuration or its platform specified
    BEFORE this work, or the recorded human override; a Compose or service file this diff adds
    licenses nothing. Where none does, stop-and-tell or ask like any other unnamed location. A
...
10. Dies with the task — agent-chosen intermediates, a report read once, a dispatched process's output
    ...
    (out-of-repo-writes.md kind 3). A task-scoped artifact one CI job hands to a later job goes through
    the platform's own established, bounded-retention artifact channel where the pipeline already uses
    one — not a repository path and not an invented directory.
<!-- END PLACEMENT TABLE (74 payload lines) -->
```
...
names the place, or a human decided before you wrote** — if nothing names the place, stop-and-tell or ask.
...
- **every Decision site asserted inside its own section slice, both trigger wording and pointer** —
  ...
  and **byte-identical to the verbatim text in Decision 13**; **all three canonical blocks — the placement table, the row-6 authority ladder and the core placement
  paragraph — asserted for marker uniqueness, declared-versus-actual payload count, and byte equality
  between the
...
- ADR 0041 present and indexed; the intended dated blocks and matching status entries asserted by name
  ...
  **every amended ADR keeps its body** — 0041 included, which takes 0042's dated block like the rest —
  ...
  error or pass vacuously. Each uses the Status-block-stripped
