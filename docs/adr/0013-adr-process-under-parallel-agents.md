...
The append-only property the rule actually protects is preserved (originals are never rewritten); agents get a legal path to correct the record; parallel ADR-writing stops colliding. This ADR's first use is its own introduction — 0000 receives an amendment note pointing here, and the pre-existing notes on 0006/0010 are reformatted into the sanctioned block. Cost: one more status value and one naming convention in `howto/adr.md`.
