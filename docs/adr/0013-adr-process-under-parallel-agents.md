...
Status: Accepted (2026-07-09). Amended (2026-08-04).
...
**Amendment (2026-08-04, see issue #78):** Decision 2 is replaced; decision 1 stands (this block is an instance of it). Merge-time numbering was never once practised: `git log --all --diff-filter=A -- 'docs/adr/DRAFT*'` returns nothing — no `DRAFT-` file has ever existed in this repo — and every ADR added after this one (0016–0026) was added already numbered, by a single-parent feature commit on its own branch, never by a merge commit renaming a draft. Each time, check 1 cleared it after confirming the number was free. The rule the log actually runs on is **verify-then-claim**: the writer claims the number when the ADR is written, having checked `docs/adr/` on `main`, every remote branch and every open PR, and records that check in the PR description. On a repo whose main session serialises dispatch, that is the same collision guarantee as merge-time renaming at a fraction of the ceremony — and a dead-letter rule is worse than either, because it quietly teaches that this log's conventions are optional. `howto/adr.md` carries the operative wording.
