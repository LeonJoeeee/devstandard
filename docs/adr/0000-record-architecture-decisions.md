...
**Amendment (2026-08-04, see 0013 as amended):** The numbering half of the block above no longer holds. Numbers are **not** assigned at merge: the writer claims the number when the ADR is written, after verifying it is free across `docs/adr/` on `main`, every remote branch and every open PR, and records that check in the PR description (`howto/adr.md`). The dated-amendment half of the 2026-07-09 block is untouched.
