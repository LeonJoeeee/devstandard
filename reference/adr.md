...
- **Claim the number when you write the ADR: the next above the highest claimed anywhere, never the lowest free** — checking *unmerged* work too, since an open branch, an open PR, or an issue that reserved one holds a number before it exists in the tree.
