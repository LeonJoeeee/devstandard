...
- **The two forms correspond, and every block must appear in the status line:** a block reading `(date, see NNNN)` pairs with `Amended by NNNN`; one citing no ADR pairs with `Amended (date)`. The status line is the index — an amendment it does not announce is one a future session will not find, so it reads the original Decision and stops. This is mechanically checkable; check it rather than trusting the eye.
