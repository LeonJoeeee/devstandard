...
Ask first whether the issue answered what actually went wrong, whether that problem is primary or
secondary, and whether the fix costs more than living with it — that is, whether this change
optimizes something that should not exist. Where the goal would be met as well by removing a rule,
a layer, a file or a step, or by leaving the problem unsolved, say so in these grounds and let the
Notes propose the smaller change, rather than judging a PR that repairs an unneeded layer only on
how well it repairs it. Recording that is not a new failure category: the Goal answer still turns
on the issue's goal.
