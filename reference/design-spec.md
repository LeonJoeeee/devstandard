...
- **Review = the existing pre-code challenge, run by the main session before implementation**: a clean reviewer (freshly spawned, no history, didn't write it — a read-only `codex exec` where Codex is installed, `reference/external-agent.md`) tries to poke holes in the spec before any implementation starts. Nothing new is added on top.
