...
- **Loosen, skip or delete a CI check to turn a red run green; re-run a failing check until it passes; read a red run as CI being unable to run.** (Two exceptions, both below: repairing an assumption your change deliberately staled, and a tracked, visible quarantine of a flake.)
