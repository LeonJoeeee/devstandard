...
- **Write only where the method allows** — add only documentation admitted by `reference/in-repo-writes.md`; otherwise use the repo, your session's scratch (the location your harness provides, e.g. `$CLAUDE_JOB_DIR/tmp` on Claude Code; a dedicated `mktemp -d` directory where it provides none), or a location a tool's convention or the repo's `CLAUDE.md` names; never an invented directory under `$HOME` (`reference/out-of-repo-writes.md`).
...
- a download, environment, or deploy root needs a home and nothing names one — no tool default, nothing in the repo's `CLAUDE.md`; picking a spot on the human's machine is theirs to decide, not yours to invent (`reference/out-of-repo-writes.md`);
