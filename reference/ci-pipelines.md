...
A self-hosted runner is the other way out, and it is not a degradation: the merge gates are unchanged and only the compute moves to a machine you own (a one-line `runs-on:` change), so no minutes are charged. Standing one up is the human's call, like topping up and going public; which operating model to register — ephemeral by default, because a persistent one makes green a weaker claim — what it costs and what a session can check are `reference/self-hosted-runner.md`. Two of those costs decide whether to reach for it at all: the machine has to be up when a PR lands — a run queued behind an offline one is not an outage and not a fallback trigger — and it must **never** be used on a public repo. When the constraint is minutes rather than a platform that is down, reach for this before the check-2 fallback (`reference/ci-cannot-run.md`).
