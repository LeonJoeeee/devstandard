...
- Files: `docs/adr/NNNN-kebab-title.md` — four digits, sequential, zero-padded, never reused. Seed the log at repo creation with `0000-record-architecture-decisions.md` (the ADR saying this project uses ADRs, containing the trigger rules above).
- **Supersede, never edit.** A changed decision = a NEW ADR stating "Supersedes NNNN"; the only permitted edit to the old one is flipping its status to "Superseded by MMMM". History is never rewritten.
- Statuses: just `Accepted` and `Superseded by NNNN`.
