...
- **The number is claimed when the ADR is written — and only after verifying it is free.** Verifying is the writer's duty, and it has four places to look, because a number can already be claimed by work that has not merged:
  ...
  - every open issue that has reserved a number for work not yet written — a real case, not a hypothetical: this method's own log has had a number whose only claim lived in an open issue, with nothing in any tree, branch or PR to find, and a check of the three places above would have collided with it.
